...
1. Conduct survey
    ...
    1. For spring biannual survey only also collect both:  
            a) `crown.illum`: 5-class scale recorded for all trees in mortality census (adopted from dendrobands protocol)  
            ![crown illum](crown_illum.png)
            b) `crown.prosition`: 4-class scale (adopted from SERC)  
